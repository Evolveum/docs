...
For easier merging to the master branch, you can use `./build.sh --local`. With this option, the script writes the `_configLocal.yml` file to your home directory.
When using this option:

* Docs directory is `$HOME/docs`.
* MidPoint directories are `$HOME/mp-*`.
* Sample directory is `$HOME/mp-samples`.

If you want to change the directories' names or paths, don't use any options and adjust the `evolveum-jekyll-theme/_config.yml` file.
