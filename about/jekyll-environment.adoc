...
[NOTE]
====
Unlike other parts of midPoint documentation, the Book (located in the `midpoint-book` repository) is, by default, built from a specific tag.
If you want to build your local changes, you need to configure Jekyll to disregard this tag:

. Go to the `docs` folder.
. In the `_config.yml` file, locate the `environment` section.
. Change `bookTag: "<tag>"` to `bookTag: ignore`.
====
