...
Macro parameter is identifier of a glossary term (e.q. `identity-management`).
...
One way to do it is to use URL `pass:[https://wiki.evolveum.com/display/midPoint/]` and add the page
name used with `wiki:` after the final slash and see where it resolves.
Then use the absolute path mostly (but not always) starting with `/midpoint/...` and place
that after `xref:`.

Example before: `wiki:Architecture+and+Design[]`

And after: `xref:/midpoint/architecture/[]`

Part inside `[...]` can be left as is.
