...
[source,asciidoc]
----
For example, this is how this resource is configured in the midpoint hogwarts demo: sampleRef::samples/demo/hogwarts/objects/003-resource-csv.xml
----

Currently, we are supporting code and CSV files.
When including a code file, it will create a code block.
...
To use it with your table, you need to add the `[.datatable-config]` block with config before the table.
Even for the default setup, it is necessary to specify the empty JSON `{}` - omitting it hides your table.
At least one empty line must be placed between the datatable configuration and the table itself.
...
That's why both the non-empty block (empty JSON literal is non-empty block for AsciiDoc) and the empty line after it is important.

Here is an example of a table with default configuration (with no JSON):
...
The options are shown in https://en.wikipedia.org/wiki/Letter_case#Kebab_case[kebab-case] - as used in HTML (but without `data-` prefix).
Use lower camel case (dromedaryCase) for JSON specification inside AsciiDoc (e.g. `pageLength` instead of `page-lenght`).
