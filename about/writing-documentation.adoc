...
The options are shown in https://en.wikipedia.org/wiki/Letter_case#Kebab_case[kebab-case] - as used in HTML (but without the `data-` prefix).
Use lower camel case (dromedaryCase) for JSON specification inside AsciiDoc (e.g. `pageLength` instead of `page-length`).
...
| `original-order`
| Preserve the original order of the rows at initialization; override `order` and `order-column` options
| false
| true, false
