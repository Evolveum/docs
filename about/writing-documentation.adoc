...
[WARNING]
====
To enable the `btn` macro rendering, you need to add the `:experimental:` flag to the page header. Not to be confused with the xref:#experimental[:page-experimental: flag].
====

You can find icon names in the midPoint web GUI using your browser dev tools. For example, in an element `<i class="nav-icon fa-solid fa-flask"></i>`, the icon name is `flask` ( icon:flask[] ), without the `fa-` prefix.
