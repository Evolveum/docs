...
In the `xref` macro, use _absolute_ paths (as opposed to relative) that point to the source file, not to the URL.
...
Docref macro (`docref`) is used to link to SchemaDoc or JavaDoc pages.
If called from a reference page, it automatically chooses the latest minor version of midpoint based on current branch.
If called from a non-reference page, it uses the default docs branch `defaultBranch` to choose the latest minor version of midpoint.

It's called with a prefix `docref:`, followed by `/javadoc/` or `/schemadoc/`, and then the path to the target page without the version part of the URL.

For example, for the JavaDoc page `https://download.evolveum.com/midpoint/4.8.9/midpoint-4.8.9-javadoc/com/evolveum/midpoint/model/api/expr/MidpointFunctions.html#getArchetype(O)`, the `docref` macro would be used like this:

[source,asciidoc]
----
docref:/javadoc/com/evolveum/midpoint/model/api/expr/MidpointFunctions.html#getArchetype(O)[optional link text]
----

WARNING: For now, the `docref` macro is without link checking, so it is possible to link to non-existing pages.
...
The macro is used in the same way as the xref:#samples-macro[samples macro].
...
----
<1> Place ToC above the article body.
<2> Limit the ToC depth up to H2 (i.e., do not list H3 and lower).
    A section level is one less than the number of `=` signs that precede the heading in the source.
    Refer to link:https://docs.asciidoctor.org/asciidoc/latest/toc/levels/[Adjust the TOC Depth | Asciidoctor Docs] for details.

The `page-toc` variable may contain several values, changing the placement of the ToC:

|====
| Value | Description

| `top`
| ToC at the top of the document, right beneath the main title. +
    Strongly preferred in the Documentation.
...
[[multiple-redirects-in-one-document]]
WARNING: There *must not be multiple redirect declarations in one document*. +
If multiple historical paths need to be kept (e.g., article moves from `/mp/path/a` to `/mp/path/b` and later to `/mp/path/c`),
list them under a single definition: +
`:page-moved-from: /mp/path/a, /mp/path/b/` +
The list is syntactically identical to `:page-keywords:`.
...
| Exact match on this attribute will put the page in the first place in search results.
...
Depending on their purpose and intended audience, various documentation pages have different formats and contents.

=== How-To Pages

These are procedural guides that show practical end-to-end scenarios.
They have the following common features:

* They are located in the context of the features they present.
For example, the xref:/midpoint/reference/tasks/synchronization-tasks/live-synchronization/create-livesync-task-howto/[] is located at the following path: `Task Management > Synchronization Tasks > Live synchronization`.
* The page name indicates what is achieved by following the how-to, for example, xref:/midpoint/reference/tasks/synchronization-tasks/live-synchronization/create-livesync-task-howto/[]:
** It starts with an imperative verb.
** Uses the "how-to" suffix.
* The procedure is presented as a numbered sequence of steps that lead to the intended result.
* How-to pages should focus on the procedure and avoid unnecessary opinions, alternatives, or recommendations that are not required to accomplish the described scenario.
...
To prevent text from breaking into multiple lines, you can use:

* `[.nowrap]\#some text#` - This is used mainly for <<referring-to-midpoint-gui-elements,icons and accompanying buttons>>.
* `\&#8209;` - Non-breaking hyphen that you can use in a text to prevent line break at the hyphen.
For example, [.nowrap]#`\btn:[Re\&#8209;generate]`# will make sure btn:[Re&#8209;generate] is not broken into two lines.
