...
(i.e. only 'groupOfNames' type of object DN's will be present in the 'memberOf' attribute ).
The memberOf and referential integrity module has to be enabled when native associations configuration is set up. Also, LiveSync is not capable
...
The attribute value, in our case `"inetOrgPerson"+memberOf -# "groupOfNames"+member`,
...
By using the above mentioned example, the connector will manage the relations between the object class `inetOrgPerson`
...
If using "Operational Attributes" in the values for the `managedAssociationPairs` configuration property,
