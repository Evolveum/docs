...
The native association configuration overrides parts of the original schema.
...
also a ‘custom’ type and some additional schema parameters are generated for such attributes.
The new ‘member’ parameter is marked as ‘not readable’ and ‘not returned’ by default.
The fetch of this attribute is not currently supported.

If you already have a schema generated before the native association configuration has been set,
