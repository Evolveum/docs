...
Starting from the connector version 3.8 the Active Directory Connector is capable of constructing reference attributes that describe the relation between resource objects.
...
has the following convention '"*Subject objectClass name*"+*Subject parameter* -# "*Object objectClass name*" + *Object parameter*'.

* "*Subject objectClass name*", being the object class representing the "Subject" of this association (the party that receives the entitlement, i.e. account).
* "*Subject parameter*", being the parameter containing a list of entitlements of the subject (i.e. group of which an account is a member of).
* "*Object objectClass name*", being the object class representing the "Object" of this association (the actual entitlement, i.e. group).
* "*Object parameter*", which is the parameter containing a list of the members of an object.
...
this also changes the way how the schema is generated by the connector. Regarding this please see <<_schema_for_native_associations>>.

As one of the limitations, the native association configuration does not handle "*primary groups*" (i.e. 'Domain Users').
There are also some other limitation to this feature, please see the <<_limitations>> section.

=== Schema for Native Associations

One of the outcomes of the setting up of the configuration for <<_native_associations>>,
...
The new ‘member’ parameter is marked as ‘not readable’ and ‘not returned’ by default, the fetch of this attribute is not currently supported.

If you already have a schema generated before the <<_native_associations>> configuration has been set up,
