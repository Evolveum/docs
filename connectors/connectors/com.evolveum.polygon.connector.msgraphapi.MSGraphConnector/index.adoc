...
Query url construction for Accounts now gives the possibility to specify complex queries using
Composite filters (‘And’, ‘Or’) and also ‘Not’ filter. There are a couple of api limitations in the
usage of filters.


- Contains filter: Altho the graph API documentation states that a contains filter functionality
...
Please see the example below: TODO
