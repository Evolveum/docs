...
This guide describes the principles and method used to develop the connectors for midPoint.


== ConnId and Polygon

MidPoint is using the link:/connectors/connid/framework/[ConnId framework] as a connector layer.
...
The ConnId framework is a continuation of Identity Connector Framework (ICF) originally developed by Sun Microsystems.
...
There are special-purpose attributes that have a specially formatted names `__LIKE_THIS__`. There are two very special identifiers `__UID__` and `__NAME__`, but there are also other attributes such as `__PASSWORD__`, `__ENABLED__` and so on.
...
Each connector object should be identified by two identifiers: \__UID\__ and \__NAME\__:
...
That means that there cannot be two objects that have the same `__UID__` or the same `__NAME__` (in one `ObjectClass`). The `__NAME__` is specified by midPoint when a new object is created.
...
Even though both \__NAME\__ and \__UID\__ are just strings, ConnId framework provides special classes for these attributes.
These are `org.identityconnectors.framework.common.objects.Uid` and `org.identityconnectors.framework.common.objects.Name`. These classes provide the constants for the `__NAME__` and `__UID__` attribute names and schema definitions ("info"). The `Uid` class is also often used in the code for type safety.
...
* link:/connectors/connid/connector-development-guide/setting-up-new-connector-project/[Setting Up New Connector Project]
...
It also means, that identifiers in midPoint appear as `icfs:uid` and `icfs:name`. Which is ugly, inconvenient and easy to confuse with native attribute names (e.g. in LDAP there is also `uid` attribute which is quite different from `__UID__`). Fortunately there is relatively simple way how to fix that.
...
| create(...)
| Object with the specified \__NAME\__ already exists.
...
| *AlreadyExistsException*
| update(...)
| Attempt to rename object to a conflicting \__NAME\__ (or other identifier).
...
It is usually a change of the \__NAME\__ attribute that is considered to be a rename.
But theoretically any attribute change may trigger rename.

The rename operation may in fact change \__UID\__.
The \__UID\__ should be immutable in an ideal situation.
But the situation is not always ideal.
Therefore if \__UID\__ is changed during a rename then simply return new \__UID\__ value from the update(...) operation.


=== __UID__ and __NAME__ are the same

There are resources that do not have `__UID__` or any similar identifier.
They just have a single mutable identifier (`__NAME__`). But ConnId framework insist that there has to be an `__UID__`. It is hardcoded into the framework.
...
* link:/connectors/connid/framework/[ConnId framework]
