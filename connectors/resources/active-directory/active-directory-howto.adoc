...
.Powersheel - username: midpoint; password: qwe.123 ; Password never expires : set ; domain: domain.example.com
[source,powershell]
New-ADUser -AccountPassword ( "qwe.123" | ConvertTo-SecureString -AsPlainText -Force) -DisplayName "MidPoint" -Enabled $true -Name "MidPoint" -PasswordNeverExpires $true -SamAccountName midpoint -Surname MidPoint -UserPrincipalName "midpoint@domain.example.com" -Path "CN=Users,DC=domain,DC=example,DC=com"
