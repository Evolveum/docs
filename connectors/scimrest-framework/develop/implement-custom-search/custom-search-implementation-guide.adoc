...
:page-description: API reference for SCIMREST search handlers in Groovy, covering endpoint configuration, response format, paging, filter support, and custom search logic.
:page-keywords: search handler API, endpoint handler, Groovy search script, filter support, paging support
...
* `chain(spec)` - Combine two attribute specifications

Value types:

* `anySingleValue()` - Any single value (alias: `anyValue()`)
...
configuration options, available objects and methods, and examples, see xref:/connectors/scimrest-framework/develop/implement-custom-search/[].