...
:page-toc: top
:page-moved-from: /connectors/scimrest-framework/develop/90-custom-search-implementation/
...
:page-description: How to implement custom search logic in a SCIMREST connector when standard REST endpoints cannot return a full object list, using indirect multi-step search.
:page-keywords: custom search, indirect search, Groovy script, search handler, object class search
...
* `filter()` - The filter that should be interpreted by this custom search
...
* `operationOptions()` - The ConnId operation options of the search
* `definition()` - The object class definition of the current search operation
* `objectClass(className)` - Returns an `ObjectClassScripting` for the specified `className`, which allows you to perform searches for objects of that particular object class
* `attributeFilter(protocolName)` - Returns a fluent builder for filters for the current object class this implementation is tied to. The `protocolName` is the name of the attribute as defined in the protocol. Only `eq(value)` is supported today.
...
* `search()` - Searches for all objects of this class without any specific filter; returns a collection of objects.
* `search(filter)` - Searches for objects of this class which match the specified filter; returns a collection of matching objects.
