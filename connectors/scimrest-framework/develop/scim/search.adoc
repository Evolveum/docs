...
:page-toc: top
:page-description: How to configure SCIM 2.0 search in a SCIMREST connector, covering supported filter declarations, limitations for non-compliant servers, and custom search.
:page-keywords: SCIM search, search filter, filter limitations, SCIM connector, search handler
...
Hovewer most real world SCIM 2.0 does not support specification fully or allow to search using any attribute, so you can specify limitations
for search handler.


.Search Handler with Limitations
[source,groovy]
----
objectClass("User") {
    search {
...
See xref:../implement-custom-search/[] for details.