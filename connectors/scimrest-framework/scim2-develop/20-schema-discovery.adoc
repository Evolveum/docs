...
== Viewing discovered schemas

Enable development mode to inspect discovered schemas:
...
This exposes `conndev_ScimSchema` object class. Search for schemas to see their JSON content:

[source,groovy]
----
objectClass("conndev_ScimSchema") {
    search {
        endpoint("/v2/Schema") {
            pagingSupport {
                request.queryParameter("startIndex", paging.pageOffset)
...
The framework maps SCIM resources to ConnId object classes:

|===
|SCIM Resource |ConnId Object Class |Example
|User |User |`User`
|Group |Group |`Group`
|Custom resource |Custom name |`Organization`
|===
