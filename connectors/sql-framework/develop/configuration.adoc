...
NOTE: `getAutoDiscoverSchema()` (used by older API surfaces) reports auto-discovery as enabled when *either* `scanTables` *or* `scanViews` is `true` — scanning only one of the two is enough to turn auto-discovery on.
