...
|Boolean
|`false`
|Exposes the discovered SQL object classes as an additional object class for schema introspection during development (see xref:schema-discovery/[]), detailed login capabilities, scripts reloaded on each operation.
...
For an unrecognized prefix the driver class is left to HikariCP, which derives it from the JDBC URL.
The driver JAR must be part of the connector bundle (see xref:custom-connector/[]).
