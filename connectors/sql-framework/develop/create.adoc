...
Keep the manifest's `operation` list empty and define a SQL-backed `+++__UID__+++` mapping (see xref:/connectors/sql-framework/develop/schema-discovery/[] and xref:/connectors/sql-framework/develop/schema-customization/[]).
