...
* Operation scripts share a single Groovy shell, so helper functions defined in one script are available to other scripts as well:
+
[source,groovy]
----
...
* *Custom mapping rules* — `SqlSchemaTranslator.addResourceRule(...)` and `addAttributeRule(...)` register additional schema-mapping rules into the rule chain that runs after the built-in rules (see xref:schema-discovery/#built-in-mapping-rules[built-in mapping rules]).
