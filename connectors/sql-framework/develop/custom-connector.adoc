...
`ManifestBasedConnector` reads `connector.manifest.json` / `.yaml` / `.yml` from the classpath and loads the declared schema and operation scripts (see xref:/connectors/sql-framework/develop/declarative-yaml/[]).
// TODO: classpath -- link to explanatory details if you decide to add it to declarative-yaml as per my suggestion there.
...
The `Messages.properties` file provides the display labels for the configuration form.
// TODO: Wouldn't the 2nd sentence deserve a bit more explanation?
