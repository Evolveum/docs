...
// TODO: Will this be valid after release of 4.11? Or 4.12?
//       We have to be careful with these warnings because they easily become obsolete and there is no automatic way to check them.
//       Also, I think this warrants :page-experimental: flag - to be removed when this warning is removed.
...
* Filtering *through* a relationship is not part of the built-in filter translation — filter on the parent object class attributes (see xref:/connectors/sql-framework/develop/search/[] and xref:/connectors/sql-framework/develop/custom-search/[] for custom needs).
* Embedded object classes are read-only from the ConnId side;
    writes to child data are a development goal (see xref:sync/[] for status notes).
// TODO: I cannot identify where this link is supposed to lead - there is no file '.*sync.*' in the sql-framework section, no such section, …
