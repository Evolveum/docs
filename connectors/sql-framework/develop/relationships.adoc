...
* Read-only flat projections of additional tables (LEFT JOINs) are not relationships — configure them with the `join` block (see xref:schema-customization/[joined attributes (read-only)]).
