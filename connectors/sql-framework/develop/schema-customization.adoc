...
Schema scripts are evaluated with a bound `objectClass` function.
Inside its closure you can map the object class to an SQL table and define attributes:
// TODO: I am unsure about the technical terminology here:
//       - "with a bound objectClass -- it sounds like the objectClass is bound to something, is that the intended meaning?
//       - "closure (of a function)" -- used multiple times in this section, is "inside closure (of a function)" really the terminus technicus for saying "within the (bounds of the) function"?
...
// TODO: Is "plain attribute" really the terminus technicus here?
//       Because if not, it sounds a bit too vague for reference material.
...
// TODO: What does the bracket say?
//       In particular, the "identity" part, I do not understand what "identity" has to do with auto-incrementing or auto-generating a value in a DB column.
//       Appears also in /connectors/sql-framework/develop/create/
...
// TODO: I would welcome some definition of the types (e.g., to understand VARCHAR vs. VARCHAR2 without having to lookup Java(?) docs)
...
----
// TODO: I may be missing something, but what purpose has an attribute without a counterpart on the remote system, or rather, why define it in the connector?
...
* if `+++__NAME__+++` is not explicitly mapped, it is derived from the `+++__UID__+++` mapping (the same underlying columns)
...
See xref:/connectors/sql-framework/develop/schema-discovery/[] for what discovery detects, and xref:/connectors/sql-framework/develop/relationships/[] for multitable attributes.