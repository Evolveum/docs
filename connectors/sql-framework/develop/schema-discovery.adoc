...
. *Composite primary key* — the first primary key column becomes `__UID__` and the remaining primary key columns are attached as additional UID columns; the composite UID value is the column values joined with a `.` separator (e.g. `tenant-1.account-42`).
