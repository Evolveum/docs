...
. *No primary key and no unique column* — the object class is still created, but without a `__UID__` mapping; no create/update/delete handler can be built for it. Exclude such tables with `scanExcludeTables` unless they are only needed read-only.
...
NOTE: `__UID__` and `__NAME__` are always exposed as `String` attributes, regardless of the underlying column type — numeric key columns are coerced transparently, and composite UID values are the key column values joined with a `.` separator (e.g. `tenant-1.account-42`).
