...
** Translates discovered (or declared) SQL tables into ConnId object classes and columns into related ConnId object class attributes.
...
** Search: ConnId filters translated to SQL predicates, paginated execution, built-in `where {}` refinement and fully custom SQL query support; read-only joined object classes (flat LEFT JOIN projections, see xref:develop/schema-customization/[joined attributes (read-only)]) are filterable and paginated the same way.
...
Scripting support::
** Custom Groovy DSL scripts for schema customization (object classes, tables, attributes) and operation handlers (search, write enablement/disablement).
...
Dialect abstraction::
** SQL generation is based on QueryDSL SQL templates auto-detected from the connected database; dialect-specific constructs (pagination, identity keys) are handled by QueryDSL's dialect-specific SQL template layer (see the https://www.querydsl.com/[QueryDSL documentation]).
// TODO: Maybe they'll fix this but otherwise I'm afraid we'll have to find another website to link here - "Firefox doesn’t trust this site because its certificate isn’t valid for www.querydsl.com.
// The certificate is only valid for: *.github.com, *.github.io, *.githubusercontent.com, github.com, github.io, githubusercontent.com."
// Maybe this one would suffice? https://openfeign.github.io/querydsl/
...
* Not strongly tied to `Groovy` with potential to replace it with a more declarative approach (YAML front-end).
...
* Schema scripts delegate to per-object-class *schema builders* (`objectClass { ... }`), which in turn delegate to attribute builders for `attribute { ... }` blocks.
* Operation handler scripts delegate to per-object-class *operation support builders* with `search { ... }`, `create { ... }`, `update { ... }`, `delete { ... }` blocks.

Because the scripts run in a shared Groovy shell, helper functions defined in one script can be reused by other scripts of the same kind (schema or handler).
