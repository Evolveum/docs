...
When basic incompatibilities are handled there is another obstacle: xref:/iam/iga/rbac/role-explosion/[role explosion]. The number of roles required to define all the necessary permission combinations grows rapidly.
...
Make sure that the xref:/iam/iga/idm-consistency/[consistency mechanism] of your IDM system is designed to handle that.
...
xref:/iam/iga/rbac/role-explosion/[Role explosion] is a common problem and definitely a dead end for an RBAC program.
...
* When choosing or implementing a solution make sure you consider a xref:/iam/iga/idm-consistency/[consistency mechanism]. It may seem as an implementation detail from the distance, but this little detail may ruin entire project.
...
* xref:/iam/[IAM Introduction documents]