...
// TODO: IGA intro, explanation of the term
// TODO: alternative terms: IDM, governance, GRC, identity security

// TODO: IGA high-level architecture

== Identity Governance and Administration Capabilities

// TODO: capabilities intro, structured, formalized, functions
// TODO: unifying terminology
...
Most common use of expressions is to customize behavior of _attribute mapping_, transforming attribute values as they are mapped from/to _identity resources_.
...
* *Schema management* mechanism maintains definition of data models in each connected system (_identity resource_).
...
Schema of such _identity resources_ is usually discovered automatically, by the means of _identity connector_.
...
* *Identity connectors* are small pieces of integration code that facilitate the connection to _identity resources_ (source and target systems).
Identity connectors are usually running on the IGA platform, accessing _identity resource_ remotely over the network, although some IGA platforms are still using agents that have to be installed on _identity resource_ systems.
...
In their pure form, identity connectors are essentially protocol adapters, interpreting operations of IGA platforms, executing them on _identity resources_ using their native protocol (LDAP, SQL, HTTP, etc.)
...
Almost all IGA platforms expose the functionality in a form of network-accessible interface (API), usually in a form of HTTP-based RESTful service.
Capabilities of the exposed API vary from IGA product to product.
Some products are based on _API-first_ approach, exposing all the functionality in the API.
