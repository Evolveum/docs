...
This page describes the data model that is used across many components, e.g. xref:/midpoint/architecture/archive/subsystems/repo/identity-repository-interface/[Identity Repository Interface], wiki:Provisioning+Subsystem[Provisioning Subsystem], wiki:IDM+Model+Subsystem[IDM Model Subsystem], etc.
