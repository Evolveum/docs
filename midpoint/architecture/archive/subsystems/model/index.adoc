...
The objects defined in the wiki:IDM+Model+Schema[IDM Model Schema] is specific for IDM model component.
...
It is only processing wiki:Inbound+and+Outbound+Expressions[Inbound and Outbound Expressions] and handles wiki:Synchronization+Examples[Synchronization Examples]. RBAC or other "advanced" mechanisms are not yet implemented.
