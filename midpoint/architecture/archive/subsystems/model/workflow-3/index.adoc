...
* xrefv:/midpoint/reference/before-4.8/cases/workflow-3/[general information about workflow feature],
...
Currently, we use https://www.activiti.org[Activiti] to manage workflow processes in midPoint.
...
For dumb processes, these tasks are active (in RUNNABLE state), waking each xrefv:/midpoint/reference/before-4.8/cases/workflow-3/legacy-pre-3-5-workflow-configuration/[processCheckInterval] secods, and actively checking the wf process instance state.
