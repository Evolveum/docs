...
The input and output data to the xref:/midpoint/architecture/archive/subsystems/repo/identity-repository-interface/[interface of the repository subsystem] are objects from the wiki:Data+Model[Data Model]. The data model objects may be represented in XML and the repository may take advantage of that.
