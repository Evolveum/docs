...
:page-toc: top
:page-keywords: inducement, entitlement, assignment
:page-description: A quick primer to help you understand the differences between inducement, entitlement, and assignment, distinct but interconnected concepts in midPoint
...
{% mermaid %}
flowchart LR
    A[User] --> |is assigned| B[Role] --> |leads to| C(Entitlement) --> |associated with| A
{% endmermaid %}
++++
...
RoleA --> |leads to| EntlA --> |associated with| UserEnd
RoleB --> |leads to| EntlB --> |associated with| UserEnd
{% endmermaid %}
++++
