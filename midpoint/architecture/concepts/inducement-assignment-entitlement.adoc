...
.Example inducement in a role. Anyone who is assigned the role obtains an LDAP account via the inducement in the role.
[source,xml]
----
<role oid="role-oid">
  <inducement>
    <construction>
      <resourceRef oid="ldap-resource-oid" type="ResourceType"/>
      <kind>account</kind>
    </construction>
  </inducement>
</role>
...
.Simplified account construction by inducement in a role
[source,xml]
----
<role oid="role-oid">
  <inducement>
...
Inducement (indirect) – The arrows from Role nodes to Entitlement nodes illustrate inducements. When a user is assigned a role, the role’s inducements automatically provision the corresponding entitlements for that user.
