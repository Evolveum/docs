...
= Simulation
:page-toc: top
:page-description: This article introduces the simulation feature and how it removes the "fear factor" from deploying new configurations or connecting new resources.
:page-keywords: simulation, test, testing, preview, development environment, task, preview mode, development configuration
...
That is where simulations come in.
...
With simulations, you safely test on the real production system, no artificial environment required.
Simulations work with import, reconciliation, recomputing focal objects and shadows, changes in roles or assignments, i.e., most of the devices your midPoint deployment uses on day-to-day basis.

Importantly, simulations do not run in an artificial sandbox;
midPoint uses the production data, it just does not change them in the simulation mode.
A simulation yields exactly the same results as if it was not a simulation, except that it is only to report what the results _would_ be.
Nothing is written to the feature:repository[repository].

== Controlling the simulation scope

You may be asking, "How do you differentiate the production configuration from the one you are preparing and want to use for a simulation?"
...
In tandem with lifecycle states, simulations use different feature:task-management[task execution] settings in regards to mode and configuration.
...
After a simulated task finishes, you get a detailed report of what would be done were it not a simulation.
...
* xref:/midpoint/reference/simulation/[] to dive deeper into simulation configuration options
* xref:/midpoint/reference/admin-gui/simulations/[] to learn about the GUI for simulations
