...
The primary goal of xref:/midpoint/[midPoint] project is to create and maintain a state-of-the-art open source identity governance and administration (IGA) system.
...
* Generic repository implementation that works with several databases (PostgreSQL, MS SQL Server, Oracle).
    ...
    ** Use of generic repository implementation is not recommended because its xref:/midpoint/release/4.9/#generic-database-support-deprecated[support is deprecated].
