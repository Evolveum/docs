...
There is an ongoing effort to specify the schema in xref:/midpoint/axiom/[Axiom], a next-generation data modeling language.
...
* xref:form-follows-purpose.adoc[]

* wiki:IDM+Means+Integration[IDM Means Integration]

* wiki:Separation+of+Concerns[Separation of Concerns]
...
Even related projects, such as xref:/midpoint/studio/[integrated development environment] are based on open source roots.
...
* wiki:Design+Notes[Design Notes]
