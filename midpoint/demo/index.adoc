...
.. link:https://wiki.evolveum.com/display/midPoint/Object+Template[User template] also determines which roles to assign to the new user.
It does so by checking the value of `subtype` user property.
This in turn comes from the HR system and was synchronized to user using link:https://wiki.evolveum.com/display/midPoint/Inbound+Mapping[inbound mapping] from the HR account.
