...
It provides a web interface where messages generated by the demo can be inspected without sending real emails.
...
If he does so, midPoint automatically delivers the notification in Italian by selecting the appropriate localized content from the Message Template.

The notification is sent by e-mail to the approver using the SMTP transport.
For demonstration purposes, the demo environment uses Mailpit as its SMTP server.
Mailpit captures the outgoing messages and provides a web interface where you can examine them without sending e-mails to external mail servers.

To view the notification e-mails:

. Open https://demo.evolveum.com/mail/[Mailpit].
