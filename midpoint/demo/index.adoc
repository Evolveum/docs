...
. Our identity engineers team are eager to automate and they already prepared configuration in _Development mode_ that will remove all orphaned account. This is a good practice when you have all accounts in order because you will effectively prevent creating new orphaned accounts because they will be deleted before anyone will have the change to start using them.  Let's check what the engineering team prepared.
