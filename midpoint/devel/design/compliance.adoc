...
What about partition management? Who would delete partiotions with append-only tables?
(ISO27001 8.15)
...
== Default Security Roles / Authorizations

Midpoint should have defined following security roles:
...
These roles should provide enough access level for working tasks as specified below.
As each implementation environments may differ, the configuration may be slightly modified based on requirements.

Security officer, security manager or auditor can be standard users as well.
So they may need authorizations to request access, process access requests and process access certifications.
The configuration of the roles below should not block from obtaining authorizations defined by other default roles.

=== Security Officer

Security officer acts as owner or administrator of the data in midPoint. Not technical configuration, but data.

Security officer can mark objects using object marks.
He can perform role cleanups - modify role names, descriptions, ownerships.
He can modify assignments of roles to users, can block users.
He can perform simulations while configuring roles.

Security officer does not perform configuration of midpoint, but he must be able to read configuration defining business rules.

Required authorizations for Security officer role::

* Full access to objects of UserType, RoleType, ServiceType, OrgType, ShadowType
* Access to certifications and cases allowing to override decisions/ approve for other
* Full access to simulations
* Run tasks (even reconciliation)
* Being able to run prepared reports
* Read only access to following midpoint configuration
    ** Archetypes
    ** Message templates
    ** Object collections
    ** Object templates
...
Access disabled to following unnecessary GUI elements::
...
Difference between Security officer and Security manager role lies in "management level". Whether the person is actively managing the data using midPoint (officer) or making decisions only and the detail changes are performed by other people.

Security manager can mark objects using object marks.
Security manager still keeps top level access of blocking user in midPoint in emergency situations.

Required authorizations for Security manager role::

* Full access to objects of UserType
* Access to certifications and cases allowing to override decisions/ approve for other
* Read only access to all other midPoint object types
    ** allowing marking objects of UserType, RoleType, ServiceType, OrgType, ShadowType
* Being able to run prepared reports
...
=== Auditor
User of this role is mainly reviewing reports and dashboards, checking status and history of objects and reviewing audit events.

The user can see all data, but can't modify it. He doesn't need to see midPoint configuration.

Required authorizations for Auditor role::

* Read only access to all midPoint object types
* Being able to run prepared reports
...
* resolve issue https://support.evolveum.com/projects/midpoint/work_packages/10198[#10198]
...
(the example of gradual policy enforcement, just with manual steps and less objects)
