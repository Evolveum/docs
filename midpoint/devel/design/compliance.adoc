...
This could be used to list all configurations that relate to a particular process, e.g. when that process is reviewed or audited. Can the "business process" be modeled as service, using assignments as references?

* Compliance checklist: dashboard-like page, that checks for presence of configuration for individual compliance frameworks. E.g. it can check for:
...
* Number of all accounts (all resources)

* Number of active accounts (all resources)

* Number of active accounts per resource (e.g. for license management)
