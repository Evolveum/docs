...
Problem: qname == qname comparison does not work well in CEL, it uses default Java comparison, cannot be customized.
Therefore, it does not handle `null` namespaces well.
Maybe we should prefer `assignment.hasRelation(relation)` anyway.
