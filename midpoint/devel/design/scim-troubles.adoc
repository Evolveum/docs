...
* `\\__UID__`, which is unique and it should be immutable if possible.
...
* `\\__NAME__`, which is provided by the user when account is created.
...
There are systems that do not have `\\__UID__` at all.
There are systems that do not have `\\__NAME__` at all.
There are systems where `\\__NAME__` does not need to be unique.
There are systems where a combination of several identifiers is needed to uniquely identify an account.
There are systems that use both `\\__UID__` and `\\__NAME__`, but `\\__UID__` is required as identifier to modify account.
There are systems that use both `\\__UID__` and `\\__NAME__`, but `\\__NAME__` is required as identifier to modify account.
