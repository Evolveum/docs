...
** #TODO What about queries on candidate owners? Some examples here?#
...
. A narrower range of pre-defined queries (defined by midPoint plus defined by the administrator before the simulation run) should be evaluated quickly.
. A wider range of generic queries may be slower, but still possible.
. There may be some exotic queries that cannot be formulated after-the-simulation.
They will require some preparatory work before the simulation in order to tag the results of individual operations with appropriate marks for later querying.
