...
The midpoint uses standard Bootstrap 4.6 and AdminLTE CSS classes whenever possible.
If a component needs a custom class, the SCSS code for that class should be places in the file:

`src/frontend/scss/*midpoint-theme.less*`
...
== Processing of CSS and javascript
...
During maven build `npm run build` is executed, which starts webpack for `production` environment.
