...
Use source comments, wiki, comments in the issue tracking system or any other method that records the reasons.
...
Right now.
If it takes longer then create an issue in OpenProject (and put the issue ID in the comment in the code so it will not get duplicated).
...
* xref:/midpoint/devel/guidelines/xsd-schema[XSD Schema]
