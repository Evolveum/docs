...
This main IGA report is described in detail xref:../reporting/www-report.adoc[here].

=== Who are the high risk / privileged users ?

Scenario::
Security officer or auditor wants to list privileged users.
...
Scenario::
IGA user wants to list users with access risk higher than some value.
IGA user wants to find users with high risk.
Security officer or auditor wants to list privileged users.
...
IGA user can filter users based on the risk value computed by algorithm (e.g. just sum) of the risk of all roles the user has assigned - directly or indirectly.
+
The list of high risk / privileged users can be provided by report or just by selection in the users view.
