...
Examples for displaying the content and functionality of the roles to business users can be found in xref:../role-engineering/app-role-design/index.adoc#_application_role_display[Application role design] and xref:../role-engineering/business-role-design/index.adoc#_business_role_display[Business role design] pages.
...
// Toto je podmnozina reportu TODO - pozriet, ktory report.
...
IGA user wants to know all business roles in which is the specified application role included (as an inducement or assignment).
This is bottom-up traversal through roles.

This information should be displayed in UI. E.g. in role object detail.

Motivation::
Finding all business roles that contains specified application role is needed while analysis of access of the application.
It is useful also during some reconstruction of access of particular application.
...
This view (or report) should display the roles starting from top level business roles and including all levels of inducements.
The roles may be duplicated in the role model display. It is natural as one application role may be included in multiple business roles.

Example of role model display::

* Business role 1
** Application role 1   -> Access to application 1
*** Metarole X
** Application role 2   -> Access to application 2
*** Metarole X
* Business role 2
** Business role 3
*** Application role 1  -> Access to application 1
**** Metarole X
** Application role 4   -> Access to application 4

Also some additional information may be displayed for each role - role archetype, owner, count of members, ...

The information may be result of some report - it may be quite expensive to compute such information each time in GUI.

NOTE: We have already role catalog displayed by org tree. It is useful in some situation, but this catalog does not display the hierarchy of the roles according the inducements.
