...
- these user stories are to be prepared, just notes here so far

=== #TODO# Define assignment parameter while role request
...
* probably GUI updates will be necessary, but we could handle this very simmilar to time constraints
    ** or we can add aditional step to request access wizard to add parameters to such parametric roles
...
=== #TODO# Add additional parameter / Modify request parameter of the role request during approval

- here specific role, and modification of approval
Examples::
* User requests role, but application owner limits assignment validity for the next 3 months.
* User requests a mailbox role, but in the process email system administrator defines size of the mailbox