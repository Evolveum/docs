...
Almost all o the tests are using wiki:Dummy+Resource[Dummy Resource]. See xref:/midpoint/devel/testing/integration/model/[Model Integration Tests] page for more details.Runs by default, e.g. *mvn clean install* without *-DskipTests* or *-DskipITs*. Use *-DintegrationTestSuite=fast* to run only the most important tests (part of "quick" build).
...
Some of the tests are also documented (see xref:/midpoint/reference/samples/story-tests/[Story Tests]).Runs in *extratest* profile (*-P extratest*) or if *-DskipStoryTests=false* is used.
