...
You can use link:https://github.com/Evolveum/midpoint-samples/blob/v4.0.1/samples/resources/csvfile/localhost-csvfile-resource-advanced-sync.xml[CSV example] as demo resource.
...
(currently there are misleading errors reported, see bug:MID-1085[], comment)
...
(currently there is an error without indication that it was in fact handled, see bug:MID-1085[], comment)
...
+
Currently this seems not to work (bug:MID-1069[]).
...
| 6. The user (step 6) should be able perform all activities with midPoint while is still logged. As soon as user logs out his access is denied because of unassigning testRole by administrator in another web browser.This is currently not functional - bug:MID-1420[]
...
(see xrefv:/midpoint/reference/before-4.8/cases/workflow-3/workflow-notifications/[Workflow notifications])
a| [source,xml]
...
. Copy schema file from `samples/schema/extension-genericsync.xsd` to `$midpointhome$/schema`.
...
<mapping>
    	...
    	<expression>
            <assignmentTargetSearch>
        		...
        		<filter>
                    <q:text>name = `orgpath.tokenize('/')[0]`</q:text>
				</filter>
...
<mapping>
        ...
        <expression>
            <assignmentTargetSearch>
                <targetType  xmlns:c="http://midpoint.evolveum.com/xml/ns/public/common/common-3">c:RoleType</targetType>
                <filter xmlns:ext="http://midpoint.evolveum.com/xml/ns/story/orgsync/ext">
                    <q:text>extension/ext:responsibility = $ext:responsibility</q:text>
                </filter>
                <createOnDemand>true</createOnDemand>
                <populateObject>
                    <populateItem>
...
link:https://github.com/Evolveum/midpoint-samples/blob/v4.0.1/samples/resources/csvfile/localhost-csvfile-resource-advanced-sync.xml[https://github.com/Evolveum/midpoint-samples/blob/v4.0.1/samples/resources/csvfile/localhost-csvfile-resource-advanced-sync.xml]
   			...
   			<searchBoxConfiguration>
   				...
   				<availableFilter>
   					...
   					<searchItem>
   						<path>c:name</path>
   						<filter>
                            <q:text>c:name contains[polyStringNorm] "ad"</q:text>
                        </filter>
                        <display>
                                <label>Name</label>
