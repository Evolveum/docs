...
It is common that identity management systems govern complex wiki:Approval[processes] based on sophisticated wiki:Policy+Rules[policies and rules]. The processes are built to maintain identity-related structures such as wiki:Advanced+Hybrid+RBAC[role-based access control (RBAC)] structures, wiki:Organizational+Structure[organizational hierarchies], project-oriented structures, self-managed organizations and all the other kinds of identity-related structures.
...
wiki:Synchronization[Synchronization] takes place almost all the time.
...
Some such situations are handled automatically by wiki:Synchronization[synchronization] mechanism.
...
We can also simply mark the violating wiki:Shadow+Objects[shadow object] with a specific policy situation marker.
...
But there are fine-grain in midPoint: recomputation of attributes values, application of roles, attribute-level reconciliation, processing of wiki:Entitlements[entitlements] and so on.
