...
It is common that identity management systems govern complex wiki:Approval[processes] based on sophisticated wiki:Policy+Rules[policies and rules]. The processes are built to maintain identity-related structures such as wiki:Advanced+Hybrid+RBAC[role-based access control (RBAC)] structures, xref:/midpoint/reference/org/organizational-structure/[organizational hierarchies], project-oriented structures, self-managed organizations and all the other kinds of identity-related structures.
