...
Mail article: wiki:Relativity[Relativity]
...
This wiki:Relativity[relative approach] is an integral part of midPoint from the very beginning of the development.
...
But midPoint makes this much more powerful then usual by the means of very flexible wiki:Mapping[mapping] mechanism.
...
E.g. an wiki:Organizational+Structure[organizational unit] can be assigned to a role which specifies on which resources to create wiki:Focus+and+Projections[a projection] of that organizational unit.
...
A handful of wiki:Advanced+Hybrid+RBAC[roles], wiki:Object+Template[object templates] and wiki:Expression[expressions] can handle situations for which other IDM systems need thousands of lines of custom code.
...
Once again we take advantage of wiki:Relativity[relative changes]. The wiki:Deltas[delta] that has to be provisioned to the unavailable system is recorded in the wiki:Shadow+Objects[shadow object] until the target system becomes available again.
...
* MidPoint has a sophisticated built-in opportunistic wiki:Consistency+mechanism[consistency mechanism]. There are several ways how midPoint can detect an inconsistency during a normal provisioning operation.
...
With wiki:Generic+Synchronization[Generic Synchronization] feature midPoint is reusing wiki:Synchronization[synchronization mechanisms], provisioning capabilities, wiki:Assignment[assignments], wiki:Mapping[mappings] and essentially almost all the other features of identity management to synchronize many object types.
...
For example a group `wizards` in AD can be mapped to midPoint wiki:Organizational+Structure[organizational unit]`Wizards` which represent the "Wizard Task Force".
...
Main article: wiki:Architecture+and+Design[Architecture and Design]
...
Because it is wiki:Architecture+and+Design[publicly available] for peer review.
...
Main article: wiki:Openness[Openness]
