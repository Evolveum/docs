...
:page-toc: top
:page-description: The demo environment provides a simple identity management solution for a fictitious organization.
...
image::deployment.drawio.svg[Deployment diagram]
...
| Dummy e-mail inbox
| Mailpit
| Simple application that mimics an e-mail inbox.
It provides a web interface where messages generated by the demo can be inspected as if sent to real inbox.
| https://demo.evolveum.com/mail/[https://demo.evolveum.com/mail/]
...
Navigate to _Cases / All approvals_ menu in _Administration_ section.
Open the case named _Assigning role "Patron" to user "Raffaello Sanzio da Urbino (raphael)_, to visualize whole approval workflow.
Switch to Workitems panel. Here you can see actual approval stage (Manager).
It is first of two approval stages (1/2).

. Open the approval stage.
Click _Approve_ button to approve the request.

. Wait few seconds and navigate to the case through _Cases / All approvals_ again.
...
. Navigate to _Certification - Campaign definitions_ menu and click the Create campaign button next to Certify user-role assignments campaign.
...
This scenario shows how to notify `francis` and `borgia` about an xref:/midpoint/get-started/demo/#governance-role-request-process[approval request].
Cesare Borgia and King Francis are busy men and rarely log in to midPoint.
As a result, your approval request might remain pending for quite some time.

To speed up the approval process, the role request event triggers a notification.
The format of the notification message is defined by a message template.

To examine the message template:

. Click *Message templates* in the left menu.
. Select *All message templates*.
. Open *Request access template HTML*.

Cesare Borgia is Italian and can set Italian as his preferred language.
If he does so, midPoint automatically delivers the notification in Italian by using the appropriate localized content from the message template.

The notification is sent by e-mail to the approver using the SMTP transport.
For demonstration purposes, the demo environment uses Mailpit to receive and display e-mail notifications.
MidPoint sends the notifications to Mailpit using SMTP, and the received messages can then be viewed in the Mailpit web interface.

To view the notification e-mails:

. Go to https://demo.evolveum.com/mail/[Mailpit].
. Select the notification message in the Mailpit inbox.
. Examine the rendered HTML message.

The same message template can be used with other notification transports, such as file or SMS.
...
As an example assign organizational unit `Department of Machines (F0200)` as this well demonstrates advanced features of organizational structure.
...
.. The types of objects you can select from are displayed in the new window.
See that you can also define assignment relation (Default or other) at this point.
Click on the tile _Organizational unit (Default)_.

.. Select the organizational unit `Department of Machines (F0200)`
Click _Add_.
