...
:page-moved-from: /midpoint/install/containers/customization/
...
This makes it safe to remove the container and create a new one by attaching the volume (changing the image version).
...
To customize the session timeout for midPoint, set the `server.servlet.session.timeout` property to a custom value.
If you do not specify units, seconds are assumed.

To set this property in a containerized midPoint instance, you can use the <<runtimevariables,`MP_SET_` mechanism>>.

In the YML file (`docker-compose.yml`), set the `MP_SET_server_servlet_session_timeout` environment variable in `services.midpoint-server.environment` to your preferred value.

.Example: setting the session timeout to 4 hours
[source,xml]
----
services:
  midpoint-server:
    environment:
      - MP_SET_server_servlet_session_timeout=4h
----
