= Docker Compose
:page-moved-from: /midpoint/install/containers/docker/, /midpoint/quickstart/quickstart-docker-compose/, /midpoint/install/docker/
:page-nav-title: Docker Compose
:page-display-order: 60
:page-toc: top
:toclevels: 4
:page-keywords:  container, docker, docker compose, database password, session time-out, session timeout, automatic logout, automatic log-out
...
* Common xref:/midpoint/get-started/install/containers/[containers] related information with options to customize the installation. +

* xref:/midpoint/get-started/install/containers/kubernetes/[Kubernetes] related information with a sample configuration. +

* Container xref:/midpoint/get-started/install/containers/customization/[customization] related information. +
...
=== Session time-out definition

The Docker Compose file can contain definition of session length using which you can control how long it takes before users are logged out of the GUI when inactive.

Add the following configuration line to the `services/midpoint_server/environment` section in `docker-compose.yml`.
The line below sets the session length to five hours.

`- MP_SET_server_servlet_session_timeout=5h`
...
* xref:/midpoint/get-started/quick-start/[]
* xref:/faq/[MidPoint FAQs]
* xref:/midpoint/get-started/install/post-install-orientation[]