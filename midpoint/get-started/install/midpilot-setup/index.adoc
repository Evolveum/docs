= MidPilot AI services setup
:page-nav-title: AI setup
...
MidPilot requires an LLM to function.
This article describes the architecture and requirements of midPilot, as well as configuration options to connect the midPilot microservices to an LLM provider of your choice.
...
Evolveum currently does not provide an LLM and the required infrastructure.
We are preparing our self-hosted LLM infrastructure but until it is ready for you to use,
you must supply your own self-hosted LLM infrastructure
with an endpoint that is compatible with the link:https://developers.openai.com/api/reference/chat-completions/overview[OpenAI Chat Completions API].
OpenAI itself is not required—any OpenAI-compatible provider or proxy works.

.Be wary of public LLM services
[NOTE]
====
Although you can employ a public LLM to use midPilot,
you should be aware of the risks.
External LLMs over which you have no control may store data you send them and use them for training,
potentially exposing your sensitive information.

We strongly suggest using self-hosted LLMs which are fully under your control.
====
...
The database must exist and be accessible before the service starts because you have to run Python Alembic database migration before launching the API server.
...
|`LOGGING__LEVEL`
|info
|Logging level (e.g., `debug`, `info`, `error`, `warning`, `critical`)
|===
...
Deploy the `midpilot-connector-gen` container (e.g., via Kubernetes, Docker Compose, Nomad job, etc.) and run `alembic upgrade head` to perform the initial PostgreSQL database migration.
...
At runtime, `hypercorn` (ASGI web server implementation for Python, used internally by the microservice) may *attempt to download or verify Python dependencies*.
...
* xref:/midpoint/get-started/try-ai-in-midpoint/[]