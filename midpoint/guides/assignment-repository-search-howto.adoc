...
image::assignment-repository-search/full-text-1.png[All five assignments]

After entering `approver` into the full-text search box the `Approver` role assignment is shown:

image::assignment-repository-search/full-text-approver.png[Assignment matching the word 'approver']

After entering `decisions` into the full-text search box the `Approver` and `Reviewer` assignments
are shown:

image::assignment-repository-search/full-text-decisions.png[Assignments matching the word 'decisions']
...
image::assignment-repository-search/custom-extension-value.png[Custom extension value]

We may do the same for e.g. `Superuser` assignment, leading to:

image::assignment-repository-search/assignments-with-additional-column.png[Assignments with additional column]
...
image::assignment-repository-search/extension-search-xml.png[Extension search using XML]
image::assignment-repository-search/extension-search-axiom.png[Extension search using Axiom]
...
By default, the assignments are sorted alphabetically by the target namefootnote:[See the
list of current limitations for the distinction between the name and the display name here.].
...
image::assignment-repository-search/search-for-system-role-assignments.png[Search for system role assignments]
