...
We consolidated our container image tagging strategy in May 2026, as described in xref:../container-image-naming/[Container Images Naming Conventions].
...
Old images were built to run as `root`, which is not the best practice.

New images are created to run as a non-root user `midpoint` with UID 1000.
