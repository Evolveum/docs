...
In this example, the file `file.csv` is copied from your host machine into the `/opt/midpoint/var` directory inside the `midpoint_server-1` container.

This is useful when you need to quickly upload files (e.g., CSV resources or configuration files) into the container.

NOTE: You can also copy files in the opposite direction (from container to host) by reversing the source and destination.
