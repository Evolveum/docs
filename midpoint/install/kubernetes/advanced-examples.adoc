...
[NOTE]
The file is available as one key of the configmap - link:https://github.com/Evolveum/midpoint-kubernetes/blob/main/advanced-examples/configmap-poi.yaml#L8-L54[111-sysconf-deployment.xml @ configmap-poi.yaml] on github.
...
.configmap with flexible authentication | link:https://raw.githubusercontent.com/Evolveum/midpoint-kubernetes/main/advanced-examples/configmap-poi-120.yaml[Github]
[source]
apiVersion: v1
kind: ConfigMap
metadata:
        ...
        - name: init-structure
          ...
          args:
            ...
            - mkdir -p /exports/icf-connectors ;
              echo "connectors directory has been created..."
...
...
[NOTE]
NFS server is described in xref:/midpoint/install/kubernetes/cluster.adoc#_nfs[Cluster deployment].
Only differences would be preparing also directory for the export.
...
.Make directory for the export on NFS server
[source]
...
      initContainers:
        - name: init-structure
          ...
          args:
            ...
            - mkdir -p /exports/export ;
              echo "export directory has been created..."
...
