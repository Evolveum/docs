...
.midPoint with repository located in the PostgreSQL
[source,kubernetes]
apiVersion: apps/v1
kind: StatefulSet
metadata:
...
.midPoint with native repository located in the postgresql
[source,kubernetes]
apiVersion: apps/v1
kind: StatefulSet
metadata:
...
.Example of the post-initial-objects - new users
[source,kubernetes]
apiVersion: v1
kind: ConfigMap
metadata:
...
.midPoint with DB auth password in the file
[source,kubernetes]
apiVersion: apps/v1
kind: StatefulSet
metadata:
...
.midPoint with persistent volume for post-initial-objets (subtree of home on PV)
[source,kubernetes]
apiVersion: apps/v1
kind: StatefulSet
metadata:
