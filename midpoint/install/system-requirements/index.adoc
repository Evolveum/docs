...
MidPoint has internal mechanisms to recover from system outages, which are utilized in this approach after midPoint restarts on a different machine.
...
As mentioned above, we always recommend using a dedicated database for midPoint.
