...
* The processing of xref:/midpoint/reference/resources/resource-configuration/synchronization/[synchronization actions] is independent from both steps and also mappings inside them.
...
* xref:/midpoint/reference/expressions/introduction/[Mappings and Expressions] are used in many places in midpoint:
...
* xref:/midpoint/reference/synchronization/introduction/[Synchronization] policy is xref:/midpoint/reference/resources/resource-configuration/synchronization/[configured] just once.
...
The xref:/iam/idm-consistency/[consistency] is an link:http://en.wikipedia.org/wiki/Achilles_heel[achilles' heel] of Identity Management.
