...
* wiki:Advanced+Hybrid+RBAC[Roles] and xref:/midpoint/reference/expressions/object-template/[object templates] can be applied to numerous types of objects, not just users.
...
* The principle of wiki:Outbound+Mapping[outbound mappings] is used both in xref:/midpoint/reference/resources/resource-configuration/[resource definitions] and in wiki:Advanced+Hybrid+RBAC[roles]. The principles are the same.
