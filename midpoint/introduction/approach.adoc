...
The somehow extreme examples are perhaps xref:/midpoint/reference/roles-policies/rbac/[meta-roles] and wiki:Assignment[higher-order inducements]. Please have a look at wiki:OrgSync+Story+Test[OrgSync Story Test] description to get an idea how complex configuration can be created by using just a handful of mechanisms.
...
The wiki:Assignment[assignments] can be used to set the policy, the wiki:Assigning+vs+Linking[links] and wiki:Shadow+Objects[shadows] track the reality and xref:/midpoint/reference/synchronization/introduction/[synchronization] is used to align them.
