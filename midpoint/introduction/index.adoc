...
E.g. the rules may take the field "organization unit" from the HR record and used that to determine which xref:/midpoint/reference/org/organizational-structure/[organizational unit] the user belongs to and which wiki:Advanced+Hybrid+RBAC[business roles] should this user have.
