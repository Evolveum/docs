...
:experimental:
:icons: font
:page-description: Define user data handling rules.
...
| Add focus
| Active
| The account from the CSV does not exist in midPoint yet, so create the user (focus) in midPoint and link it to the account shadow.
...
As for spaces vs. dashes—you can use either, but dash signs are generally the safer option.
