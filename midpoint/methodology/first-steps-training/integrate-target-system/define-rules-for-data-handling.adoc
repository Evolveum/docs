...
The process is similar to what you have done in xref:/midpoint/methodology/first-steps-training/connect-source-system/define-mapping-and-synchronization-rules/[] but since the AD is a target application, you need to tailor the settings accordingly; most notably, you will learn to create inbound mapping rules limited only to correlation.
...
* xref:/midpoint/reference/admin-gui/resource-wizard/object-type/mapping/advanced-mappings/[]
...
[.nowrap]#icon:arrow-right[] *xref:/midpoint/methodology/first-steps-training/integrate-target-system/reconcile-ad-accounts/[]*#