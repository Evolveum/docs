...
You can now activate the orphaned account deactivation (TODO: turning on `marked as disposable` -> `disable` / `delete`) to deactivate/delete all orphaned accounts (marked as disposable) in the target system using midPoint.
+
...
+
When you are satisfied with the mappings, (TODO!) change the mappings to actually set the values.
