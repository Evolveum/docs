...
TODO When to actually clean up the orphaned/inconsistent data? In Step 2 or Step 3?
...
** But we should have some way of cleanup - maybe "Delete all identities" functionality we already have, *##NEW FEATURE REQUIRED ??? Deleting shadows with additional information (e.g. marks) may be bad##* just put it somewhere more "visible"
...
*Solution*: Some ideas are described <<new-resource-wizard,here>> and <<how_to_connect_hr_to_midpoint,here>>. TODO move here.
...
NOTE: This is irrelevant for external authentication and/or if no self-service will be used in midPoint.
...
NOTE: This is irrelevant if no approvals will be used in midPoint.
