...
+
You should also review the list of orphaned accounts (accounts in Active Directory not having an owner in midPoint - meaning not related to HR data).
Such accounts may actually be not just orphaned, but also system accounts (which need to be kept protected by midPoint) or accounts suffering the data inconsistencies (which need to be resolved in further iterations).
The accounts need to be reviewed manually and actions can take place either in the AD, or in midPoint, both manually or automatically.

TODO IN PROGRESS: what is left: orphaned accounts, system accounts, data inconsistencies, etc.
