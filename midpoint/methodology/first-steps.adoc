...
* Status (active, former employee, alumni, etc.)
Ignore other fields for now.
We will get back to them later.

. *Import users* to midPoint, using HR data.
Select appropriate algorithm for username.
...
. *Correlate AD accounts* with midPoint users.
If you have employee numbers stored in the AD, then use that for correlation.
If you do not, use generated midPoint usernames as correlation identifier.
Run the _reconciliation_ task on AD resource (TODO!).
Then have a look at the results (TODO!).
+
...
TODO: what is left: orphaned accounts, system accounts, data inconsistencies, etc.
