...
<<explicitly_marking_accounts_for_decommissioning,_Mark_ the undesired ones as decommissioned>> to be deactivated eventually.

. *System (service) accounts*:
For all accounts that are crucial for Active Directory, we need a different decision.
Either let the AD administrators to remove the accounts if they are unnecessary or <<explicitly_marking_accounts_as_protected,_mark_ the system accounts as protected in midPoint>> to keep track of them, but ignore them otherwise by midPoint.
