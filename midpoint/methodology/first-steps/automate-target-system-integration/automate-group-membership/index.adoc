...
In this guide, we do not work with roles much and have users of only one archetype (_Person_), but all users are in the _all_users_ group in the target LDAP resource.
...
Once you verify the group is assigned to the new 90XX users and nothing else changes (as it should not), you can put the configuration to production and wait for the recurring HRIS import task on production to pick up the users without the _all_users_ group membership and assign them to the group.
...
+
image::ldap-ui-all-users-group.webp[]
...
+
image::ldap-user-lcallahan-association-ad-group-membership.webp[]
