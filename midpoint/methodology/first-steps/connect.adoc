...
* icon:id-card-clip[] *Administrative status*
    ...
    ** The activation mapping controls account status in the target system based on the effective status of their focal object in midPoint.
    ** *Lifecycle state*: _Proposed_
* icon:user-slash[] *Disable instead of delete*
    ** *Lifecycle state*: _Proposed_
    ** This mapping has no configurable options.
    ** The purpose of this mapping is to ensure accounts don't get deleted immediately,
        but are rather deactivated first and
        deleted only after the grace period set in Delayed delete mapping expires.
* icon:clock[] *Delayed delete*
    ...
    ** Accounts marked for deletion will be deactivated first and deleted no sooner than after the time set here elapses.
        ...
        *** For production, sensible values are likely much longer (weeks or months).
...
Having inspected and verified the changes as desired,
turn the proposed changes into reality.

For that to make it happen, you need to move all the mappings from the _Proposed_ lifecycle state to _Active_.
