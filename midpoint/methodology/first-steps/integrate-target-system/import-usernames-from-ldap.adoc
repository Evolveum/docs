...
You already have both simulation and production reconciliation tasks defined from the reconciliations you have done previously as part of this guide.
...
* Refer to xref:/midpoint/reference/tasks/synchronization-tasks/import-and-reconciliation/gui/[] for guide on working with tasks in GUI.
* You can also simulate the effects of your configuration changes on a single account using the xref:/midpoint/reference/tasks/synchronization-tasks/import-and-reconciliation/gui/#simulate-import-of-a-single-object[import preview] functionality.
...
Use the reconciliation task with the _Production_ configuration and leave the execution mode empty.

[NOTE]
====
No incorrect data in your target system will be fixed yet because you have yet to configure the LDAP target resource outbound operations.
