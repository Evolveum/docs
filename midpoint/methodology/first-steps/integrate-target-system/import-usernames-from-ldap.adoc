...
. Adjust source and target resource mappings to enable importing usernames from LDAP.
    .. In the HRIS resource, set the _empnum-to-name_ inbound mapping strength to to _weak_.
...
=== Add new mapping to LDAP for usernames
...
This is the default and it is useful when a user's username changes, for instance.
