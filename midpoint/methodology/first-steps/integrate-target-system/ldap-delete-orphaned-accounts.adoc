...
You will first put it to the _Proposed_ lifecycle state to enable testing; once tested, you will put it to the _Active_ state.
...
. Set the lifecycle state of the _delete-unmatched-resource-object_ rule to _Proposed_.
    ** Rule: _Delete resource object_ reaction to the _Unmatched_ situation
...
Refer to the xref:/midpoint/methodology/first-steps/integrate-target-system/reconcile-ldap-accounts/[] module to refresh your knowledge on working with reconciliation tasks.
...
image::../ldap-delete-unmatched-accounts-simulation-result.webp[]
...
. Switch the xref:#activation-rules[activation rules] to the _Active_ lifecycle state.
. Run the production reconciliation task.

// TBC with inspection of the deactivation process; currently, I have some config error in MP as the accounts are deleted immediately from LDAP
// This might also be moved to later stages to let learners move faster to the "rewarding" provisioning phases
