...
The production reconciliation will not delete any orphaned and malicious accounts because you have not yet activated the xref:/midpoint/methodology/first-steps/integrate-target-system/define-rules-for-data-handling/#define-synchronization-rules[synchronization rule] for that.
...
|====
| Account                                                     | Mark            | Note
| `cn=Ana Lopez,ou=users,dc=example,dc=com`                     | Correlate later | The CFO with a wrong employee number in AD
...
[.nowrap]#icon:arrow-right[] *xref:/midpoint/methodology/first-steps/integrate-target-system/import-usernames-from-ldap/[]*#