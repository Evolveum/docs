= Kick-off: Make a deployment plan
:page-nav-title: Plan your deployment
:page-display-order: 70
:page-toc: top
:experimental:
:page-moved-from: /midpoint/methodology/first-steps/kick-off/
...
In the xref:/midpoint/methodology/first-steps-training/[training that follows this methodology], we use a simple resource that exports CSV files to act as an HR system.
...
You will see further down the lane how to temporarily use a target system as a source for selected attributes.

[NOTE]
====
In the training, we simulate an Active Directory (AD) server using an LDAP server and use that as a target resource.
====
...
Set modest goals that you can achieve within a few weeks.
...
The IGA project is open-ended;
you cannot say that it will be "done" some day.
With the ever-changing business and IT environment, it is very likely that at least some of the IGA operations will become part of your ordinary operational routine.
...
Put the theory into practice with the First steps training.

== Next steps

[.nowrap]#icon:arrow-right[] *xref:/midpoint/methodology/first-steps-training/[]*#