...
. Start import phase (repeat until satisfied)
.. Edit mappings (`givenName`, `familyName` etc, ##TODO## `lifecycleState`; no mapping for `name` yet) - *strong mappings*
...
.. FYI: Accounts are imported as users, *no `name`, no `fullName`*
...
. End of import phase (repeat until satisfied)
...
. *Start correlation phase: repeat until satisfied*
// moved from above
...
.. End of account marking phase (optional)
...
. *End of correlation phase* ##TODO we finished here 13.5.2022##

. set reactions for `UNLINKED` -> `link` and `LINKED` -> synchronize and keep `UNMATCHED`, `DELETED` and `DISPUTED` without any reaction

. FYI: Most accounts are in `UNLINKED` and `LINKED` (manually linked accounts) situation.

. run reconciliation again to *link the accounts to midPoint owners*

. *TODO enable identity matching for `DISPUTED`*

. FYI: All linkable accounts are in `LINKED` situation or in progress of identity matching.

. Start attribute correlation phase (repeat until satisfied)
...
. End attribute correlation phase (repeat until satisfied)

. *TODO TODO run (the same?) reconciliation report to see what usernames will it generate in midPoint - simulation?*

. FYI Shadows are still marked as decommissioned/do not touch.
...
. FYI: Accounts are now updated in target system based on mappings, except the marked accounts

. *TODO TODO TODO somewhere here add correlation to HR?*

==== Automate

. Start username import phase if this is the resource to import username from (repeat until satisfied)
...
. End username import phase (repeat until satisfied)

. FYI: Now we have all *existing* AD usernames in midPoint, we can consider the identifier unique.

. *Define and turn on username generator in object template! Otherwise we cannot automate onboarding.*
