...
. mark unmatched or other accounts (linkable, but maybe some special) for later review ("do not update"), if we do not know what to do with them. (We are not sure if we can decommission them. Let's postpone the decision.)
...
... Kind, intent are optional; not needed for the first iteration
