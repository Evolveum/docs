...
.Additional shadow marks notes (2.2.2023) - to be discussed
====
...
|No
|No
a|We know, that this account should be deleted.
But not yet, not until someone "approves" it.
...
NOTE: I had an idea to allow explicit deletion of resource objects where shadow has this mark, but it would be not very nice - something that does not allow the delete operation would allow "explicit delete operation"...?
...
Perhaps correlation identifier is wrong. +
Ignore thise account for now. +
...
|No
|No
a|Allow synchronization of this account to midPoint (e.g. linking account to owner, importing login from target such as AD), but disallow any outbound modifications.
...
|No
|No
|*Does not make sense. No shadow to mark. Policy to achieve "no modification of existing content"?*
...
|Yes
|No
|*Does not make much sense for specific account. We would need to set this in advance. We can have disable instead of delete*
...
.Aliases vs Multiple marks

We can have either `Incorrect source data` (which is of "class" `Protected`) or we can have `Protected` + `Incorrect source data` additional "mark". Something similar to structural and auxiliary archetypes...
...
* automatically, by ##policy (specified where?)##
** Example: set `To be reviewed` shadow mark + mayber even create case for all `UNMATCHED` accounts automatically. When administrator decides, he/she will set another mark `Do not set any other marks` to prevent loops and `UNMATCHED` will behave normally (without marking and case creation).
