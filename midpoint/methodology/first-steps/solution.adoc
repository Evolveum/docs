...
. run reconciliation report on account level: what is different in target system (unmatched etc.) Which accounts are extra?
.. mark unmatched accounts for later decommissioning
.. mark unmatched or other accounts for later review, if we do not know what to do with them (we are not sure if we can decommission them)
...
.. run reconciliation report (interactive?) again and again during clean up
.. *TODO TODO prepare inbound mapping from target to midPoint user? In comparison mode as well?*
...
... TODO maybe that inbound should be removed later? When? Or we just make the mapping strength `weak` as we do not have any value yet in midPoint users and we will always have the value in the future! (But this will work only once.)
.. *TODO TODO run reconciliation report to see what names will it generate in midPoint - simulation?*
...
. Repeat reconciliation until satisfied, keep marked shadows until further resolution. They will not be touched by midPoint yet.
.. *TODO possible problem with UNMATCHED->DELETED situation*
.. *TODO possible problem with UNMATCHED->"PROTECTED" situation*
. switch mappings from "comparison" mode to TODO: normal/strong etc. (midPoint is still not synchronizing changes from source)
. Run reconciliation (final time) to let midPoint do the desired changes
. Run actions on to-be-decommissioned accounts (disable, delete, ...)
. Optional: do something with the accounts marked for later review
