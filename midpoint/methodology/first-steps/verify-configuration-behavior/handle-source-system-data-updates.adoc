...
In this module, you will verify that you can actually change data in the source HRIS system and that these changes are picked up by midPoint correctly.
...
. You can check the data on the target LDAP system by two means:
    ** In Geena's midPoint user profile, go to [.nowrap]#icon:male[] *Projections*#, select the target resource, and inspect the attribute values.
    ** Go directly to the link:http://localhost/phpldapadmin[target LDAP system interface], open Geena's account details, and confirm the changes there.

=== Canonical approach to keeping track of changes
...
. Select [.nowrap]#icon:magnifying-glass-chart[] *Audit Log Viewer*# in the left-side navigation menu.
