...
For this delayed delete, midPoint uses the _disabled instead of delete_ and _delayed delete_ xref:/midpoint/methodology/first-steps/enable-target-system-provisioning/#delete-ad-accounts-with-a-grace-period[activation rules you have set up earlier].
...
[.nowrap]#icon:arrow-right[] *xref:/midpoint/methodology/first-steps/verify-configuration-behavior/provision-new-attributes-to-target-resource/[]*#