...
In this module, you will see that even mappings used in production can be safely updated, tested using simulations, and then re-deployed to production in the updated form.
...
The issue with the values from HRIS is that beside the actual position name, they contain an ugly internal codes, like `124#CEO`.
Your goal in this module is to remove the codes and fill the midPoint and AD `title` attributes with clean values, such as `CEO`.

The trick to learn here is that you need to keep the old configuration in production during the testing up until the point when you are ready to seamlessly replace it on produciton with the new configuration.
...
While it is true that you already have an inbound mapping in the HRIS for the `job` attribute, you cannot simply update it.
You would either forfeited the option to test the new configuration using simulations (because the mapping is _active_, i.e., on production),
or you would put it back to the _Proposed_ lifecycle state, which would mean it would not be available on production during the testing phase.
...
Firstly, put the old inbound production mapping on HRIS to the _Deprecated_ lifecycle state.
...
.Deprecated mapping is still used on production
...
.New mapping first needs to be tested. See below for the script.
[cols="5*", options="header"]
|=====================================================================================
| Name              | From resource attribute | Expression | Target  | Lifecycle state
...
[TIP]
====
The script parses the value using '#' character and returns the second item.
For `123#CEO`, it returns `CEO`.
To be on the safe side, it takes care of anomalies as well:
...
[TIP]
====
You do not need to change the outbound mapping to AD because it will use the new value corrected in midPoint by the HRIS inbound mapping.
====

== Test the new mapping behavior

With the new _proposed_ configuration ready, it is time to test it.

. In the HRIS resource, run the simulated development import task you have prepared there already.
. After the task finishes, click btn:[Show simulation result].
. In the *Resource object affected* tile, you should see changes to all users mapped from HRIS to midPoint.
    Click [.nowrap]#*More info* icon:arrow-circle-right[]#, open an item in the list, and inspect the changes made to the item.

[NOTE]
====
Simulated development task evaluates all _active_ and _proposed_ configuration items, but makes no permanent to the data.
At the same time, the real scheduled reconciliation task executes all _active_ and _deprecated_ configuration items,
...
. Set the lifecycle state of the _job-to-title_ mapping to _Archived_.
. Set the lifecycle state of the _job-to-title-nice_ mapping to _Active_.
...
Once the recurring HRIS reconciliation task processes the change, have a look at the AD projections in user profiles in midPoint to see the new prettified job titles are there.
...
This module concludes the section of the First steps guide dedicated to confirmation the functionality and resilience of the setup you have created.
Up next is the last section fo this guide, dedicated to overriding wrong source data.