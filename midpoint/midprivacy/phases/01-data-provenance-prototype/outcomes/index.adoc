...
That just strenghten  our feeling we are digging in a new territory and people are not fimiliar with it yet.

From the day one we know that metadata will help with midPoint internal features as very convenient side-effect.
Tracing the origing of the data as well as tranformation on the way is significant help with debuging identity management processes.
...
During of delta updates of such attributes was impossible to evalues if a particular value should be removed or not, because we didn't know if was provided byt the other source or not.
With provenence this problem is becoming trivial.


At the end of the current phase of the midPrivacy project we have prepared a technical demo demonstrating our result.
...
Nevertheless some people are seeing the value from them even with current limmited capabilities.
