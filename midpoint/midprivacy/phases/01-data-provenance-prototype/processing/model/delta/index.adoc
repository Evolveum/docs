...
with D~d~ and deleted if it matches any value there. This can be written as I -~del/RVDI~ D~d~ (see
<<../comparing-values/index.adoc#definition-del,here>>).

Also, when adding values that have equivalents already present in I, we first remove those equivalents.
(Effectively, it is a kind of "add-or-update" operation.) Let's denote existing equivalents as I~r~ = I &cap;~RVDI~ D~a~.
