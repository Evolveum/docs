...
Let's try https://www.postgresql.org/docs/current/ddl-partitioning.html[partitions first] and test the performance.
...
TODO: Check PostgreSQL https://www.postgresql.org/docs/current/ddl-inherit.html[table hierarchy].
...
* Audit performance considerations? what is missing now? what about audit archiving (unsearchable?)
...
Drowbacks: No way to distribute ID creation, but we accept this.
...
Existing audit, although separate, can still stay in `repo-sql-impl`.
Unused Hibernate entities for audit are still used for automatic DB creation, especially handy for H2/tests.
** Common SQL support (current "pure" package from repo impl) should be extracted.
