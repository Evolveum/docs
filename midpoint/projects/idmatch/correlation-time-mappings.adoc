...
.A condition that finds an owner for an account based on `hrId` = `employeeNumber` condition
[source,xml]
----
<correlation>
    <q:equal>
...
This means that we need to correlate `hrId` attribute of an account to the `employeeNumber` property of an existing user.
...
.Inbound mapping that fills-in `employeeNumber` from the value of `hrId`
[source,xml]
----
<attribute>
    <ref>ri:hrId</ref>
...
The problem with this approach is that the information about the relation between `hrId` and `employeeNumber` has to be maintained
on two distinct places (that may not be physically close to each other).
...
.Inbound mapping that fills-in `employeeNumber` from the value of `hrId` (with more complex computation)
[source,xml]
----
<attribute>
    <ref>ri:hrId</ref>
...
.A condition that finds an owner for an account based on a relation between `hrId` and `employeeNumber`
[source,xml]
----
<correlation>
    <q:equal>
...
This is obviously not very maintainable for the long run.
...
.A condition that finds an owner for an account based on the value of `employeeNumber` computed by a mapping
[source,xml]
----
<correlation>
    <q:equal>
...
The `$focus/employeeNumber` refers to the employee number that was mapped by the inbound mapping from the `hrId` attribute.
So, the knowledge that `hrId` maps to `employeeNumber` has to be now present at a single place only: in the mapping.
...
.Sets the default execution phases of all inbound mappings for given object type
[source,xml]
----
<schemaHandling>
    <!-- ... -->
...
If not specified, the default setting is clockwork-only execution.

The phases can be specified also at the level of individual mapping, e.g.

.Sets the execution phases for a given mapping
[source,xml]
----
<attribute>
    <ref>ri:hrId</ref>
