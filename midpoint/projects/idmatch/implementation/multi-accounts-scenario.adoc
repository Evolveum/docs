...
itself, so it may keep various (unrelated) records for a single physical person.
...
In this case, the Student Information System is represented by a simple CSV file with the following structure:
...
question is: where to put the data? The usual answer is "use extension items", but it seems not applicable here. We
...
Like this very busy student that studies four programs, and works for the Department of Computer Science:

.User `smith2` with five assignments
image::screenshot-1.png[]

The assignment contains the personal data from the particular account.
...
2. The partial date (copied from individual accounts) are stored in assignments: in
...
The correlator and cases configuration is the same as in the xref:implementation.adoc[simple matching scenario], with one
