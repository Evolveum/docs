...
But no xref:../axiom/existing-languages-analysis/[existing schema language] that we could find seems to be suitable for our needs.
...
We had to build up a whole new xref:../axiom/concepts/[conceptual model].
...
But the effort was successful and xref:../axiom/spec/[Axiom 0.1] was born.
...
xref:../metadata-multiplicity-problem/[metadata are inherently multi-valued].
...
* xref:../axiom/spec/[Axiom specification]

* xref:../metadata-multiplicity-problem/[Metadata Multiplicity Problem]

* xref:../outcomes/[Outcomes of midPrivacy: Data Provenance Project]
