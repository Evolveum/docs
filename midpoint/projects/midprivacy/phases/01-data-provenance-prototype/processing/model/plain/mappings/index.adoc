...
The main output is basically the same as in link:../expressions/[expression evaluation]: a delta set triple. However, mapping
...
See link:../expressions/[Expression evaluation] description.
...
These parameters are part of mapping definition but are treated by upper layers. See link:../mapping-sets/[mapping set
evaluation].
