...
* FullText filter (textInfo), including storage of the info (Virgo)
...
** They should be included only when asked for specifically for extensions
`ObjectSelector.path`=`c:attributes` and `GetOperationOptions.retrieve`=`INCLUDE`. - (Tony)
* Support for various get/search/modify options
* Audit service - WIP
* Audit service - add iterative search
* Query playground: `executeQueryDiagnostics()`
* Multi-node safety for `UriCache` and `ExtItemCache`
* Retry mechanism (if/where necessary)
* Better OperationResult treatment/review (code review)
