...
After first run the password can be found in the log.
In case of docker you will see the log on console or by the requesting the log of the container.

The password is generated following the password policy. To find to password look for the text in the log:

.Example of the log record (the password will be different)
----
Administrator initial password (except double quotes): "Ec5s !f7a"
----
...
In case you are running one of there version there is following default credentials.
