...
Volumes are used to store persistent data outside the container filesystem.
...
One way to do this is to copy the file directly into the running container using Docker Desktop.
...
Open Docker Desktop and navigate to your project (in our example `windows_quickstart`).
Select the `midpoint_server` container and open the *Files* tab.
...
Drag and drop the CSV file from your host system into this directory.
After copying the file, verify that you can see it in the `/opt/midpoint/var` folder on the `Files` tab.
