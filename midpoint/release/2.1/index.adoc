...
* xrefv:/midpoint/reference/before-4.8/security/audit/#database-table-auditing[SQL table][Auditing to SQL tables] (see also xrefv:/midpoint/reference/before-4.8/security/audit/[basic information about auditing])
...
. Strong mappings fail in presence of replace deltas for mapping targets (bug:MID-1067[])

. Weak mappings are not processes correctly in modify situations (bug:MID-1068[])

. Two consecutive modifications may produce strange results when resource is down (bug:MID-1054[])
