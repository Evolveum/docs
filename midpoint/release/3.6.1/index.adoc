...
Majority of the work on the _Comenius_ release was done by the https://www.evolveum.com/[Evolveum] team.
...
Therefore we can assure reliable midPoint upgrades only for https://evolveum.com/services/[midPoint subscribers]. This section provides overall overview of the changes and upgrade procedures.
...
MidPoint 3.6.1 data model is essentially backwards compatible with both midPoint 3.5 and midPoint 3.5.1. However as the data model was extended in 3.6.1 the database schema needs to be upgraded using the xrefv:/midpoint/reference/before-4.8/upgrade/database-schema-upgrade/[usual mechanism].
