...
.. Make note of all tasks which will be suspended.[TIP]
====
...
. Back up midPoint installation directory (MIDPOINT\_INSTALLATION\_PATH)[NOTE]
====
...
.. Update existing installation[source,bash]
----
...
.. (Optional) If one needs to count objects in midPoint before proceeding with the database upgrade, the following can be used:[source,bash]
----
...
It will not export the audit records.[source,bash]
----
...
Generally the number should be in between 2 and 8 +
 +
...
The following command will import the (previously) exported midPoint repository from a (compressed) archive.[source,bash]
----
...
* Upgrade to midPoint 3.7.2. Then export data using regular ninja tool.
In that case there is no need to use ninja37.

* Upgrade to midPoint 3.7.2. Export data from midPoint 3.7.2 using the ninja tool.
Create fresh installation of midPoint 3.8. Apply any customizations in midPoint home directory.
Import the data.
...
This is a modified version of ninja tool from midPoint 3.7. The ninja37 is modified to properly put the definitions in the export files.
