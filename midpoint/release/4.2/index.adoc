...
** xref:/midpoint/reference/misc/reports/configuration/#import-report[Custom data imports] (a.k.a. "reverse reports") (experimental)
...
** xref:/midpoint/reference/resources/maintenance-state/[Resource maintenance mode (contributed)]

** xref:/midpoint/reference/resources/multiaccounts/#outbound-mappings[Outbound multiaccounts]

** xref:/midpoint/reference/synchronization/linked-objects/[Linked objects]
...
** xref:/midpoint/reference/schema/archetypes/configuration/#multiple-archetypes[Support for multiple archetypes]
...
** xref:/midpoint/reference/security/authentication/flexible-authentication/configuration/[Authentication policy for node group]

** xref:/midpoint/reference/misc/reports/configuration/[Expression profiles for GUI/report column expressions]



* Internals and Development

** xref:/midpoint/reference/security/audit/#separate-repository-configuration-for-audit[Decoupling of audit and repository databases]
...
Those databases will be supported for some time (possibly long time), but xref:/midpoint/reference/repository/repository-database-support/[support for them will be eventually removed]. It is strongly recommended to use PostgreSQL instead.
...
* xref:/midpoint/reference/before-4.8/legacy/soap/[IDM Model Web Service Interface] (SOAP) was removed.
