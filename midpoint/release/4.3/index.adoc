...
Release {release-version} is a thirty-fourth midPoint release code-named _Faraday_. The {release-version} release brings performance improvements, user interface and diagnostic improvements and major internal improvements.
...
** Improved authorizations for basic/advanced search
...
** Improved monitoring of stalled tasks

** Storing more operation errors in the objects

** wiki:Live+sync+error+handling+strategy+HOWTO[Retries of failed live synchronization operations] (experimental)
...
** JSON in repo - experimental? doc? (MID-6303)
...
* Support for Microsoft Internet Explorer is deprecated since midPoint 4.2.

* Support for MySQL and MariaDB is deprecated since midPoint 4.1.
...
* Release of Java REST client ...

* Overlay projects and other associated artifacts ...
...
Therefore we can assure reliable midPoint upgrades only for link:/midpoint/support/subscription-sponsoring/[midPoint subscribers].
...
* LDAP ad AD connectors were upgraded to the latest available version 3.2.
...
This was aligned in midPoint 4.2.
...
Most changes were related to the link:/midpoint/midscale/[midScale] effort.
