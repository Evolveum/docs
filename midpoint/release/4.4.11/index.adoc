...
Release {release-version} is the eleventh maintenance update for midPoint 4.4.x version family code-named _Tesla_.
The {release-version} release brings mainly security bugfixes and dependencies.
...
== Changes with Respect to Version 4.4.10

** Updated to latest API-compatible Spring 5.3.39, Spring Boot 2.7.18, Apache CXF 3.5.11
*** This updates fixes various suite of CVEs
*** Newer versions are not backwards compatible and are supported in MidPoint 4.8.x and newer
** Updated Tomcat to 9.0.102 which fixes CVE-2025-24813
** Updated bundled AD / LDAP Connector to 3.5.1, which fixes security issues in Mina Core and commons-text. 

== Changes with Respect to Version 4.4.9
** Fixed issue with task notifier not getting error message for composite task handlers (e.g. reconciliation). See bug:MID-10026[].
** Fixed the issue with unassigning role members to produce the action only for the selected relation. See bug:MID-9936[].
** Fixed the issue with assignment counter on the object details page. See bug:MID-10099[].
** Fixed fetching accounts with different association definitions in different intents. See bug:MID-9910[].
** Fixed closing multi-node tasks when some nodes are not available. See bug:MID-10021[].
** Fixed updating invisible fields during validation of items. See bug:MID-10091[].
** Fixed displaying error messages with technical details via new configuration options. See bug:MID-9993[].

To see full list of fixes see https://support.evolveum.com/projects/midpoint/work_packages?query_props=%7B%22c%22%3A%5B%22id%22%2C%22subject%22%2C%22type%22%2C%22status%22%2C%22priority%22%2C%22assignee%22%2C%22customField6%22%5D%2C%22hi%22%3Afalse%2C%22g%22%3A%22%22%2C%22is%22%3Atrue%2C%22tv%22%3Afalse%2C%22hl%22%3A%22none%22%2C%22t%22%3A%22priority%3Adesc%2Cid%3Aasc%22%2C%22f%22%3A%5B%7B%22n%22%3A%22customField6%22%2C%22o%22%3A%22%3D%22%2C%22v%22%3A%5B%2287%22%5D%7D%5D%2C%22ts%22%3A%22PT0S%22%2C%22pp%22%3A20%2C%22pa%22%3A1%7D[Evolveum Issue Tracker].
...
** Fixed inappropriate applications of weak mappings when a resource is down. See bug:MID-9861[].
** Fixed saving properties of additional connectors. See bug:MID-7918[].
** Fixed associationFromLink and behavior regarding dead shadows. See bug:MID-9468[] and bug:MID-9487[].
...
* PostgreSQL 15, 14 or 13 (deprecated).
...
* PostgreSQL 15, 14, 13 (deprecated).
Support for PostgreSQL 10 is deprecated, it is very likely it will be removed soon.
...
* Please check if there is a need to add authorizations to specific users due to link:#_behavior_changes_since_4_4_7[behavior changes since 4.4.7].
