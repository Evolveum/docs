...
with (optionally) time-based partitined tables allowing for very fast audit data removal.

Task management system was significantly updated during midScale project. The major improvements are related to
the ease of complex task configuration, enhanced ability to distribute tasks across the cluster (this includes task
...
validity scanning, or generic partitioned tasks) is not supported. Please use activity-based
configuration instead to achieve that functionality.
...
* Dependency for support of saml2 authentication module was change to https://github.com/spring-projects/spring-security/tree/main/saml2/saml2-service-provider[saml2-service-provider]. Midpoint support old functionality of saml2 module but few configuration attributes aren't used in new dependency. Unused configuration attribute was tagged as deprecated in schema of sml2 authentication module. TODO
...
. To overwrite or customize default midPoint tables (e.g. All users, All roles,...) xref:/midpoint/reference/admin-gui/collections-views/configuration/#default-collection-views-since-midpoint-4-4[`identifier` attribute must be set correctly]. E.g. for All Users identifier `allUsers` must be used.

. Configuration for _virtual containers_ was improved. Now it is possible to define _virtual container_ on different panels, also custom ones. This required a changed in the configuration, where `container` definition was moved from `objectDetailsPanel/container` to `objectDetailsPage/panel/container`.

. GUI was significantly changed in 4.4 - tabs on details pages were replaced with details navigation menu. Old configuration for tab customization won't work with the new design. It has to be adapted to the new design and structures, look at the xref:/midpoint/reference/admin-gui/admin-gui-config/#examples[examples].
...
* Tasks should be re-imported because their run-time data structures have been changed. Moreover, bucketed,
multi-node and partitioned tasks have to be manually or semi-manually adapted to the new activity-based
configuration language. "Change execution" task should be checked for changed default object type.
...
Upgrade from midPoint 3.9.x or older to midPoint {release-version} is not supported directly. Please upgrade to midPoint 4.0.x first, then upgrade to midPoint 4.4.
...
* Archetypes (for tasks, reports,...): adapted configuration to changes in GUI.

* Tasks (cleanup, validity, and trigger scanner): migrated from legacy to the new (activity-based) configuration.
...
1. When single-run task encounters a fatal error, it is no longer closed. It is suspended instead.
This is to allow resuming and continuing with the activity that failed. (There can be more than
...
to `AssignmentHolderType` (but only when new activity-based configuration style is used).
