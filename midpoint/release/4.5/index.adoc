...
Release {release-version} is a TODO midPoint release code-named _Nightingale_. The {release-version} release brings identity matching capabilities, support for OpenID Connect, notification, user interface and other improvements.
...
TODO: Point to initial objects in `config` directory of the distribution (and link to relese github for live version).
