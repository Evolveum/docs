...
This includes new, cleaned-up structure of object type definitions (including encompassing synchronization configuration right into them), better handling of default `kind` and `intent` values, and the possibility to globally override object class definitions.
