...
NOTE: This section is relevant to the majority midPoint deployments.
It refers to major things that were removed or significantly changed in this version.
...
However, if you mention `CapabilityCollectionType` in your configuration (typically by including `xsi:type="c:CapabilityCollectionType"` declaration), you have to change this to `"cap:CapabilityCollectionType"`.
...
NOTE: This section describes changes in the behavior that existed before this release.
...
You may at least scan through them.
...
* Handling of shadows in "reaping" state has been changed.
...
(We mention this change here, as there may be deployments that rely on "dry run" doing nothing with the shadows, even if the account is in fact missing.)
...
* Audit import is more forgiving, no longer fails on unrecognizable data: bug:MID-7913[], link:https://github.com/Evolveum/midpoint/commit/a17f3544[a17f3544].
This is more robust but the administrator may overlook data loss because of schema incompatibility.
...
The newly-introduced `CacheInvalidationListener` can declare events it is interested in.
...
- Shadow classification and tag generation functionality now resides in the `provisioning-impl` module, see link:https://github.com/Evolveum/midpoint/commit/7296dd15[7296dd15].
To implement that, part of expression support was moved down from `model-common` to `repo-common` module, see link:https://github.com/Evolveum/midpoint/commit/71105192[71105192].
...
- https://github.com/Evolveum/prism/commit/8efe7400 anyIn filter #TODO Tony#
