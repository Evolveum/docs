...
** Configuration for xref:/midpoint/reference/admin-gui/self-service/#profile-page[Sel-service Profile page] was separated from administration object's (logged in entity, so user, service,...) details page configuration.
...
** Archetype, role, and GUI customization changes are almost immediate, does not require logout to take effect. Users are logout automatically when
   their administrative status is disabled. When user permissions (eg. role assigned / unassigned) are changed, this takes immediate effect.
