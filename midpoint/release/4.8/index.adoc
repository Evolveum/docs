...
** Two-steps wizard for xref:https://docs.evolveum.com/midpoint/reference/admin-gui/resource-wizard/#attribute-mapping[attribute mappings] configuration was introduced. For now, attributes shown in the first and the second step are hardcoded and can only be hidden by the configuration.
