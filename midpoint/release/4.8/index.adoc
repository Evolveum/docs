...
* There are small improvements in Request Access feature usability.

* It is now possible to use a collection view for the All Accesses panel.
...
* Helper functions xref:/midpoint/reference/concepts/query/axiom-query-language/query-language-in-groovy/[`midpoint.queryFor()` for Groovy] to use Query language directly from scripts were added.
...
** ScriptedSQL connector was updated to use Groovy 4.
** Groovy-based connectors needs to be updated to use Groovy version 4. #TODO explain better @Tony#
...
* `610-service-identity-recovery.xml`: Identity recovery service was added. This service plays a role of the midpoint principal during Identity recovery flow.
+
_Action required:_ None.
This object will be automatically created.
...
+
_Action required:_ #TODO @Viliam#
