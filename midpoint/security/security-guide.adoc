...
User are advised to use recent build from a xref:/midpoint/get-started/install/bare-installation/using-support-branch/[support branch] for their release.
...
See xrefv:/midpoint/reference/master/expressions/expressions/security/[Expression Security] and xref:/midpoint/reference/expressions/expressions/profiles/configuration/[Expression Profile Configuration] page for details.
...
When exporting reports in CSV format, midPoint intentionally preserves data in its original, unmodified form.

As a result, values starting with certain characters (such as `=`, `+`, `-`, or `@`) may be interpreted as formulas by some spreadsheet applications.
This behavior is a characteristic of the CSV format and spreadsheet software, not to midPoint itself,
and may pose a risk if CSV files are opened with default spreadsheet settings that allow formula evaluation.

If CSV export is considered a security risk in a given environment, midPoint provides authorization-based controls to restrict report export functionality.
This can be limited to a trusted set of users only.
This approach allows organizations to mitigate the risk without modifying exported data.
Detailed configuration guidance is available in xref:/midpoint/reference/misc/reports/security-note-csv-export[].

== Platform and Dependencies

MidPoint is an application running on existing computing platform consisting of operating system and Java platform as two major components.
