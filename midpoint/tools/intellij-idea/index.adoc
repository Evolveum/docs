...
MidPoint requires a Java platform to run on.

// file in /midpoint directory
...
JDK (Java development kit, sometimes called SDK) is necessary for building;
JRE (Java runtime environment) is not enough.
...
Detailed instructions on how to set up PostgreSQL database are available in the xref:/midpoint/reference/repository/native-postgresql/usage/[configuration] for native PostgreSQL repository.
