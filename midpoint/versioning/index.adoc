...
The development process is guided by the regular pace of minor releases which bring new features.
There is one minor release every autumn (in the October-November time frame).

Maintenance and patch releases are dedicated to bugfixes.
Source code for such releases is split from the _master branch_ into a xref:/midpoint/get-started/install/bare-installation/using-support-branch/[_support branch_] at the point where a minor release is finished.
...
* xref:/midpoint/get-started/install/bare-installation/using-support-branch/[Using Support Branch]
