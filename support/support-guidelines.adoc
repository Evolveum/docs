...
When it comes to xref:/midpoint/reference/diag/creating-a-bug-report/[reporting a bug] or xref:/community/feature-request/[creating feature request] the same basic principles apply.
...
xref:/midpoint/reference/security/security-guide/[Security issues].
| *Immediate* (hours)
...
However, the priority system is the best we can offer.
...
Then, the developer might jump right into fixing the bug instead of spend hours on finding the specific combination of configuration causing the problem.
...
If the issue report is sensitive and it may put others at risk then you can use our responsible disclosure mail address *security@evolveum.com*. See xref:/midpoint/reference/security/security-guide/[Security Guide] page for more details.
...
If midPoint happens to misbehave and there is a potential for harm, then such issue has to be fixed as soon as possible.This also applies to xref:/midpoint/reference/security/security-guide/[security issues] regardless of project phase.
