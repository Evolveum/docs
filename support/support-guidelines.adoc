...
If the issue has `Target versions` set, then it is likely to be addressed in that version.
...
However, what it is possible to estimate, is the planned target version.
The fix will be provided at last with the xref:/midpoint/roadmap/[release of the version] set in the `Targert versions`.
